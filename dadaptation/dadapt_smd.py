...
class DAdaptSMD(torch.optim.Optimizer):
    r"""
    Implements SMD with D-Adaptation automatic step-sizes. Leave LR set to 1 unless you encounter instability.
    Notes on standard SMD:
      SDM q = 2 -> SGD
      SMD q = 3 : use same lr as SGD 
      SMD q = 10 : acts like infinity norm : standard lr is 1e-6 to 1e-8 to learn without DAdapt but here works even with lr = 1
            ...
            for p in group['params']:
                ...
                if momentum != 0:
                  if 'momentum_buffer' not in state:
                      ...
                      buf.mul_(ck).add_(grad)
                  else:
                    buf = state['momentum_buffer']
                    buf.mul_(1-ck).add_(grad, alpha=ck)

                grad = buf   
                # x step
                update = (group['q'])* (torch.abs(p.data)**(group['q']-1)) * torch.sign(p.data) -  grad
